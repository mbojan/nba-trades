...
Processing the files created by Zachary Ricardson (in `data-src`) to produce data frames and igraph objects for analysis.


# Team and franchiese names

Based on data on standings

```{r}
team_franchise <- readRDS(here::here("data", "standings.rds")) %>%
  distinct(
    season_from,
    season_to,
    team_acronym = purrr::map_chr(strsplit(full_team_name, " "), tail, 1)
  ) %>%
  mutate(
    franchise = case_when(
      # TODO: map season-teams to franchises
      season_to <= 1978 & team_acronym == "Braves" ~ "Clippers",
      season_from >= 2004 & season_to <= 2014 & team_acronym == "Bobcats" ~ "Hornets",
      
      TRUE ~ team_acronym # else
    )
  )
```
...
From data on standings extract info on assignment to divisions

- Copy 2018-2019 season to 2019-2020

```{r}
divisions <- readRDS(here::here("data", "standings.rds")) %>%
  distinct(
    season_from,
    division = purrr::map_chr(strsplit(division, " "), 1),
    team_acronym = purrr::map_chr(strsplit(full_team_name, " "), tail, 1)
  )
divisions <- bind_rows(
  divisions,
  divisions %>%
    filter(season_from == 2018) %>%
    mutate(season_from = 2019)
)
```
...
graphlist <- lapply(
  ...
  function(s) {
    ...
    # Nodes
    n <- data.frame(
      name = sort(unique(c(e$P1, e$P2)))
    ) %>%
      left_join(
        divisions %>%
          filter(season_from == s) %>%
          rename(div = division),
        by = c("name" = "team_acronym")
      )
    g <- graph_from_data_frame(e, vertices = n, directed=FALSE)
    E(g)$weight <- count_multiple(g)
    g <- simplify(g)
    E(g)$weight <- sqrt(E(g)$weight)
...
```{r graphlist-save}
saveRDS(graphlist, file = here::here("data", "wgl.rds"))
```
