...
keywords: ["trade networks", "NBA", "sports", "Exponential-family Random Graph Models", "weighted networks"]
format: 
...
```{r fig-trades-per-year, fig.cap="**Number of Trades by Season.** (standardized to a per team rate)"}
edb %>%
...
@fig-trades-per-year presents the trend of the average trades per season. Since, the number of teams varies across the window, we standardize these values to represent the number of trades *per team* in each season. This pattern shows a roughly U-shaped trend, with the highest rates of trades (nearing 2 per team) occurring early and late in the period and the lowest rates occurring in the early 1990s. Notably, this low-point came *after* the introduction of unrestricted free-agency (in 1988), which allowed players to sign with any team after their contract expires. As a corollary, @fig-trades-per-team presents the frequency distribution of trades per team across the full observed window. There are some teams (e.g., the Spurs) who consistently trade less frequently, and others who are much more trade active (e.g., the 76ers).
...
```{r fig-trades-per-team, fig.cap="**Average Trades by Team.** (standardized to a per season rate)"}
...
```{r tbl-mm-1976-2004}
#| tbl-cap: "**Mixing Matrix for the period 1976 - 2004**"
mm_1976_2004 <- edb %>%
...
```{r tbl-mm-2005-2019}
#| tbl-cap: "**Mixing Matrix for the period 2005 - 2019**"
mm_2005_2019 <- edb %>%
...
```{r fig-alphas, fig.cap="**Divisional Homophily in NBA Trades by Season.** (as computed by Moody's $\\alpha$-index)."}
...
```{r fig-seasonal-ergms, fig.cap="**Homophily Coefficients and Standard Errors from Weighted Exponential Random Graph Models.** (including base-rate effects)"}
with(
...
To summarize the results from Figure [-@fig-seasonal-ergms] in a single model, we also fit a pooled ERGM across all seasons, including a base-rate effect for each season's trade volume, similarly to the binary ERGM mentioned above.[^modelSpecification] These results are presented in Table [-@tbl-model1-table-short]. In this final specification, we find an aggregate same-division homophily effect (coefficient 0.006, standard error of 0.059, z-statistic of 0.102), that again shows no avoidance pattern. Fitting similar models to the periods 1976-2004 and 2004-2019 separately leads to almost identical results.[^periodModels]


```{r tbl-model1-table-short}
#| tbl-cap: "**Valued ERGM fitted to pooled seasonal data**. The table excludes seasonal constants, complete table is presented in Table [-@tbl-model1-table] the Appendix."
withr::with_options(
 list(knitr.kable.NA = ""),
...
```{r tbl-model-binary-table}
#| tbl-cap: "Full results for the binary ERG model fit to pooled data."
...
```{r tbl-model1-table}
#| tbl-cap: "Full results for the weighted ERG model (Conway-Maxwell-Poisson reference) fit to pooled data."
