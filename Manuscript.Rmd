...
\sum_{d=1}^S \theta_d g_1 (y, d) +
\theta_h g_2 (y))}{\kappa(\theta_s, \theta_d, \theta_h)}$$

where the sufficient statistics are:

- $g_1(y, s)$ -- number of trade participations in season $s$
- $g_1(y, d)$ -- number of trade participations by teams in division $d$
- $g_2(y)$ -- number of trades involving teams from the same divisions

The model is constrained to within-season dyads only.
