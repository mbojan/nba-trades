...
```{r alphas, eval=FALSE}
nl <- lapply(graphlist, intergraph::asNetwork)

alpha <- lapply(nl, OR2, "div") %>%
  bind_rows(.id = "season") %>%
  mutate(
...
```{r seasonal-ergms, eval=FALSE, fig.cap="**Homophily Coefficients and Standard Errors from Weighted Exponential Random Graph Models.** (including base-rate effects)"}
# TODO: include
```
...
```{r model1-table-short, eval=FALSE}
 withr::with_options(
   list(knitr.kable.NA = ""),
...
Table: Full results for the ERG model fit to pooled data.
